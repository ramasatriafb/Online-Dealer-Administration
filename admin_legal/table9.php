<?php
	include "../connect.php";
	$sql= "select * from daftar_pengajuan a, daftar_dealer b,kodepos c where b.id_kodepos = c.id_kodepos";
	...
	if(!$_POST['date_awal']=="" && !$_POST['date_akhir']=="")
		$sqltahun = " and a.waktu_legalisir between '$_POST[date_awal]' and '$_POST[date_akhir]' ";
		else 
		$sqltahun=" ";
	...
	$hasil=mysql_query($sql.$sqlkode.$sqltahun." order by a.waktu_legalisir asc limit ".$page.",10");
	...
	echo '</table></div></div></div><div class="top-row" >
          ...
          <div class="field-wrap">
          <input type="number" style="width:15%" min="1" name="page" max="';if(ceil($count9/10)==0)echo 1;else echo ceil($count9/10); echo'" value="'.$_POST['page'].'"/><span>Dari '.ceil($count9/10).' halaman</span>
          </div>
        </div>';
?>