...
if(!isset($_SESSION)) {
	...
	$sql4 =mysql_query("UPDATE channel_h3 SET no_sp3d ='$_POST[no_sp3d]' WHERE id_channel='$kode_dealer'");
	$sql5 =mysql_query("INSERT INTO daftar_pengajuan ( id_admin, kode_dealer, tanggal_pengajuan, jenis_pengajuan, channel, status_channel, nama_sv, nama_manager, nama_divhead, jam_pengajuan, perpanjangan ) VALUES ('$id_admin', '$kode_dealer', '$date', 'Pengangkatan Dealer Tetap', 'H3', 'Tetap', '$_POST[sv]', '$_POST[manager]', '$_POST[divhead]', '$time', '$_POST[tgl_akhir]')");
	$query = mysql_query("select tgl_efektif,tgl_akhir from channel_h3 where id_channel ='$kode_dealer'");
