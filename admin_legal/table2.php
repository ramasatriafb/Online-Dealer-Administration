<?php
	...
	$hasil=mysql_query($sql.$sqltanggal.$sqljenis.$sqlkode.$sqlnama.$sqlchannel." order by tanggal_pengajuan desc limit ".$page.",10");
	...
	while ($baris = mysql_fetch_array($hasil)){
		...
		<td align=center>";if($baris['comment_sv']||$baris['approval_sv']||$baris['comment_manager']||$baris['approval_manager']||$baris['comment_divhead']||$baris['approval_divhead']||$baris['legalisir_admin']) echo "Acknowledged"; else echo "In Progress"; echo"</td>
	...
	$hitung=mysql_query($sql.$sqltanggal.$sqljenis.$sqlkode.$sqlnama.$sqlchannel." order by tanggal_pengajuan desc");
	$count1 = 0;
	...
	echo '</table></div></div><div class="top-row" >
          ...
          <div class="field-wrap">
          <input type="number" style="width:15%" min="1" name="page" max="';if(ceil($count/10)==0)echo 1;else echo ceil($count/10); echo'" value="'.$_POST['page'].'"/><span>Dari '.ceil($count1/10).' halaman</span>
          </div>
        </div>';
?>